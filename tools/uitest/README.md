...
| `[nl-app] runner: started` | 파이프라인 실행기가 시작한 뒤 | 실행 시나리오의 출발점 |
| `[nl-app] runner: http <주소>` | 인바운드 HTTP 서버가 열린 뒤 | `curl` 을 쏘기 전에 기다린다 |
| `[nl-app] runner: error <노드> <메시지>` | 실행기가 오류를 낼 때 | 실패를 기다리는 시나리오에서 |
| `[nl-app] runner: stopped` | 실행기가 멈춘 뒤 | 정지 확인 |

`runner:` 마커에 값 이벤트는 넣지 않았다. 초당 수십 줄이 되면 `wait-log` 가 훑는 로그가 금세
커져 다른 마커를 찾는 것이 느려진다. 오류는 틱마다 같은 것이 다시 오므로 **바로 앞과 같은 줄이면
건너뛴다** — 같은 이유다.
