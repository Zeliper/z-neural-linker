...
**미지 필드는 보존된다.** 문서 구조체 대부분에
`#[serde(flatten)] extra: BTreeMap<String, Value>` 가 있어, 더 새로운 버전이 만든 모르는 필드도 열고 저장하면
그대로 남는다(비어 있으면 직렬화에 나타나지 않는다).

| 있음 | 없음 |
| --- | --- |
| `ProjectFile` · `Project` · `ProjectSettings` · `ModelDef` · `Node` · `TrainConfig` · `RunRecord` · `DatasetSpec` · `PayloadSpec` · `Field` · `Pipeline` · `PNode` · `Link` · `GuiLayout` · `WindowSpec` · `Widget` · `BuildSpec` · `BundleManifest` | `Graph` · `Edge` · `EpochMetrics` |

**태그 열거형에는 붙이지 않았다.** `#[serde(tag = "type")]` 인 `LayerKind`·`Source`·`Sink`·`Transform`
같은 것들은 `flatten` 이 `type` 키를 잡아먹어 변형을 고르지 못하게 된다. 이들은 구조체 필드로
한 단계 들어가 있으므로 바깥 구조체의 `extra` 가 대신 지켜 준다.

이 앱이 그 값을 해석·편집하지는 않으며 `diff_ops` 도 `extra` 를 비교하지 않는다.
`ProjectFile::newer_than_app()` 이 참이면 호출자가 "보존되지만 편집할 수 없다" 는 경고를 띄운다.

남은 셋은 시험이 고정해 둔다(`these_structs_still_drop_unknown_fields`). 넓히면 그 시험이 깨지므로
이 표와 함께 고치면 된다. 엔진 관점 왕복은 `nl-engine` 의
`unknown_fields_survive_training_and_a_save` 가 본다 — 학습까지 돌린 뒤에도 일곱 층이 살아남는지.
