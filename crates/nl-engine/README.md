...
**한 프로세스에 학습은 하나다.** `start` 는 이미 도는 학습이 있으면 거부한다 — 여러 학습이 같은
장치 메모리를 두고 다투면 둘 다 느려지거나 함께 죽는다. 정말 필요하면 `TrainRequest::allow_concurrent`
를 켜서 명시한다(기본 `false`). `active_count()` 가 지금 도는 수를 주므로 앱은 버튼을 비활성화할 수
있다. 자리는 RAII 가드가 들고 있어 **학습 스레드가 패닉해도 돌아온다.**

**추론은 막지 않는다.** `Session::load`/`run` 은 학습 중에도 열린다. CPU 에서는 안전하고 GPU 에서는
같은 장치일 때 느려질 뿐이라, 학습을 보면서 중간 결과를 시험해 보는 흐름을 막을 이유가 없다.
