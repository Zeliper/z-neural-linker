...
//
// ONNX protobuf 메시지 정의. `onnx.proto3` (ONNX v1.13.1, IR_VERSION 8, opset 18 세대) 를
// prost 로 컴파일한 결과다. **생성 결과를 커밋해 두므로 빌드에 `protoc` 도 코드 생성도 필요 없다**
// (tract-onnx 가 쓰는 것과 같은 방식).
//
// 재생성: `scripts/onnxgen/README.md` 참고. 생성기는 `protox`(순수 Rust protobuf 컴파일러) +
// `prost-build` 라 protoc 바이너리가 없어도 돈다.
//
// 원본 `onnx.proto3` 는 ONNX 프로젝트(Apache-2.0) 저작물이다.
...
#![allow(clippy::all)]
#![allow(missing_docs)]
// onnx.proto3 의 주석에 `<TBD>` 같은 문자열이 있어 rustdoc 이 HTML 태그로 읽는다.
