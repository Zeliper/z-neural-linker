...
`Source::HttpServer` 에 `tls`(인증서·키 PEM 경로)를 주면 리버스 프록시 없이 https 로 연다. 경로는
프로젝트 폴더 기준 상대 경로이고 그 밖으로는 열리지 않는다. 인증서가 잘못되면 포트를 열기 전에 실패한다.

TLS 는 **인증을 대신하지 않는다**. 바깥에서 닿는 주소에 토큰 없이 여는 것은 TLS 가 있어도 거부한다.
토큰은 번들에 박히므로 앱을 받은 사람이 다 볼 수 있다 — 서버로 돌릴 때는 `NL_HTTP_TOKEN[_<포트>]` 로
실행 환경에서 새로 준다. 검증기는 바깥 주소를 평문으로 여는 구성에 경고를 띄운다.

인증서를 배포물에 함께 담지 말 것. 개인키가 들어간 `.nlapp` 은 그 자체가 유출이다. `nl build --tls-cert`
는 **경로만** 매니페스트의 프로젝트에 적고 파일은 넣지 않는다.

그래서 배포 앱은 인증서를 두 곳에서 찾는다: ① 작업 폴더(번들을 푼 임시 폴더) ② **실행 파일이 있는 폴더**.
번들에 인증서가 없으니 실제로 닿는 자리는 ②다 — 설치한 기계의 실행 파일 옆에 `certs/` 를 두면 된다.
작업 폴더는 실행할 때마다 새로 만드는 임시 폴더라 거기에 두어 봐야 남지 않는다.

`nl tls-cert <폴더>` 가 자체 서명 인증서와 키(0600)를 만든다. Windows 에는 유닉스 권한 비트가 없으니
키 파일을 사용자 폴더 밖에 두지 말 것.
