...
>
> **킬 스위치는 빌더에만 있습니다.** 배포판(`nl-runtime`)에는 없습니다. 다만 배포 앱의 마우스/키보드
> 싱크도 **기본은 꺼져 있습니다** — 빌드 설정에서 `입력 무장` 을 켜야 실제 입력이 나갑니다
> (`nl build --arm-input`). 켜고 배포하면 앱 상단에 `⚠ 입력 무장` 이 뜨고 시작할 때 로그로도 알립니다.
> 무장한 앱을 배포할 때는 GUI 에 정지 버튼을 꼭 넣으세요.
