...
중 무엇이어도 됩니다. 시험용 자체 서명 인증서는 이렇게 만듭니다.
...
빌더에서는 프로젝트 폴더가 기준이고, 배포 앱에서는 위의 작업 폴더가 기준입니다.

`--work-dir` 을 쓴다면 인증서를 `local/` 에 두고 이렇게 적는 것이 가장 단순합니다. 번들을 갈아 끼워도
파일이 남고, 인증서를 갱신할 때 앱을 다시 빌드할 필요가 없습니다.

```json
"tls": { "cert_pem": "local/server.crt", "key_pem": "local/server.key" }
```

인증서를 앱과 함께 배포하고 싶다면 번들 에셋으로 담고 `assets/` 로 시작하세요. 다만 인증서를 바꿀 때마다
앱을 다시 빌드해야 합니다.
