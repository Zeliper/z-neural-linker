...
`Ctrl+Z` 되돌리기, `Ctrl+Shift+Z`(또는 `Ctrl+Y`) 다시 실행, `Ctrl+B` 아웃라인, `Ctrl+Alt+B` 인스펙터,
`Ctrl+J` 하단 도크. 캔버스에서는 `F` 전체 보기, `Ctrl+A` 전체 선택, `Ctrl+D` 복제,
`Del`(또는 `Backspace`) 삭제.
...
그 사이 상태바에는 `장치 확인 중…` 이 보이다가 끝나면 `장치 자동 → Intel(R) UHD Graphics 630 (CFL GT2)`
처럼 실제 이름으로 바뀝니다.
...
- **XOR 분류 (MLP)** — `Input[2] → Linear 16 → ReLU → Linear 2 → Output`. 합성 데이터 1000개,
  학습 50 에포크가 1~2분에 끝납니다. 파이프라인과 GUI 까지 붙어 있어 전 과정을 볼 수 있습니다.
  입력 두 값은 **−1~1** 범위이고 정답은 `(x₁ > 0) ≠ (x₂ > 0)` 입니다. 그래서 시험할 때는
  `[0.8, -0.8]`(정답 1) 처럼 **0 에서 떨어진 값**을 쓰세요 — `[0, 1]` 은 학습이 본 적 없는 경계라
  결과가 아무 쪽으로나 나옵니다.
- **사분면 분류 (CNN)** — `Input[1,8,8] → Conv2d 8 → ReLU → MaxPool → Flatten → Linear 4 → Output`.
  8×8 흑백 이미지에서 밝은 사각형이 어느 사분면에 있는지 맞춥니다. 합성 데이터 2000개, 30 에포크.
  추론 API 포트는 XOR 과 겹치지 않게 `127.0.0.1:8800` 입니다.
...
맨 위가 **템플릿**(여러 레이어를 한 번에), 그 아래가 레이어 분류입니다. 분류는
**입출력 → 완전연결 → 합성곱 → 풀링 → 활성화 → 형상 → 정규화(통계) → 정규화(드롭아웃) → 병합 →
임베딩 → 순환 → 어텐션** 순이고 레이어는 모두 20종입니다. 항목에는 기본 설정이 함께 보입니다 —
`Linear  → 64`, `Conv2d  16ch k3×3 s1×1 p1×1` 처럼요.
...
`Output` 노드를 여러 개 두면 모델이 값을 여러 개 냅니다. 이때 **`Output` 노드의 이름이 곧 필드
이름**입니다. 파이프라인의 HTTP 응답 JSON과 GUI 의 `모델 출력` 바인딩이 그 이름으로 값을 찾습니다.

그래서 다출력 모델에서 이름이 비었거나 둘이 같으면 인스펙터와 **문제** 탭에 경고가 뜹니다. 오류가
아니라 경고라 학습·실행은 되지만, 받는 쪽에서 어느 값이 어느 것인지 구분할 수 없게 됩니다.
출력이 하나뿐이면 이름을 비워 둬도 됩니다.
...
빈 곳을 우클릭하면 레이어 분류 서브메뉴 위, `레이어 추가` 바로 아래에 **`템플릿`** 이 있습니다.
자주 쓰는 사슬을 통째로 놓습니다.

| 템플릿 | 놓는 것 | 설정 |
| --- | --- | --- |
| 잔차 블록 | `Linear → ReLU → Linear` 에 입력을 더하는 `Add` | 폭 |
| 트랜스포머 블록 | 사전 정규화 셀프 어텐션 + 피드포워드, 각각 잔차 | `d_model`, 헤드 수, FF 배수 |
| 합성곱 블록 | `Conv2d → BatchNorm → ReLU → MaxPool` | 채널 수 |

메뉴를 고르면 설정 창이 뜹니다. 값이 형상 규칙에 맞지 않으면 (예: `d_model` 이 헤드 수로 나누어떨어지지
않으면) 아래에 이유가 뜨고 `놓기` 가 회색이 됩니다.

**노드를 하나 선택한 채로** 템플릿을 놓으면 그 노드가 블록의 입력에 자동으로 이어집니다. 창에 어디에
붙는지 한 줄로 적혀 있습니다. 선택이 없으면 떨어진 채로 놓이니 직접 이으면 됩니다.

템플릿은 레이어가 여러 개여도 **되돌리기 한 번**에 전부 사라집니다. 잘못된 설정으로 놓았으면 `Ctrl+Z`
한 번 누르고 다시 놓으세요.

### 순환과 어텐션

팔레트의 **순환**(`LSTM`·`GRU`)과 **어텐션**(`MultiHeadAttention`)은 시퀀스를 다룹니다. 입력 형상이
`[L, D]` 여야 합니다. `L` 은 길이(토큰 수·시점 수), `D` 는 한 자리의 특징 수입니다.

| 레이어 | 설정 | 출력 |
| --- | --- | --- |
| `LSTM`·`GRU` | 은닉 크기 `H`, 양방향, 시퀀스 전체 출력 | 켜면 `[L, H]`, 끄면 `[H]`. 양방향이면 `H` 가 2배 |
| `MultiHeadAttention` | 헤드 수, 드롭아웃 | `[L, D]` 그대로. `D` 가 헤드 수로 나누어떨어져야 합니다 |

**양방향**은 앞뒤 두 방향을 돌려 마지막 차원에서 이어 붙입니다. 그래서 은닉 크기 64에 양방향을 켜면
출력이 `[B, L, 128]` 이 됩니다. **시퀀스 전체 출력**을 끄면 마지막 시점만 남아 `[B, H]` 가 되는데,
뒤에 `Linear` 를 붙여 분류할 때 이 모양이 편합니다.

글자를 다루는 흔한 사슬은 이렇습니다.

```
입력 [32]          ← 문자 32개짜리 시퀀스 (페이로드의 Tokenize 가 만듭니다)
  → Embedding      vocab 28, dim 32     → [B, 32, 32]
  → LSTM           h64, 시퀀스 출력 끔  → [B, 64]
  → Linear         → 2                  → [B, 2]
  → 출력
```

페이로드 쪽에서는 `Text` 필드에 `Tokenize` 변환을 걸어 문자열을 인덱스 시퀀스로 바꿉니다. 인덱스는
1부터 시작하고 0은 패딩 겸 미지 문자라, `Embedding` 의 어휘 수는 **글자 수보다 1 크게** 잡아야 합니다.

### ONNX 로 내보내기

모델 선택 줄의 **`ONNX 로 내보내기…`** 가 지금 모델을 `.onnx` 파일로 씁니다. 왼쪽 아웃라인에서
모델 이름을 우클릭해도 같은 항목이 나옵니다. opset 17, 배치 차원은 `B` 기호로 두는 동적 축입니다.
다른 프레임워크(onnxruntime·Netron·TensorRT)로 넘길 때 씁니다.

버튼이 회색이면 이유가 툴팁에 있습니다. 형상 오류가 남아 있거나 레이어가 하나도 없을 때입니다.
형상이 통과하지 못한 그래프는 내보내지 않습니다 — 읽는 쪽에서 깨질 파일을 만드는 것보다 낫습니다.

가중치가 아직 없으면 눌렀을 때 안내가 뜹니다. 학습을 한 번 돌리고 나면 정상 종료 시 자동으로
적용되므로 그대로 내보낼 수 있습니다.

쓰기가 끝나면 **노드 수 · 가중치 수 · 파일 크기**가 토스트로 뜹니다. 여기서 노드 수는 우리 레이어 수가
아니라 ONNX 연산자 수입니다. `Linear` 하나가 `MatMul` 과 `Add` 두 개가 되는 식이라 더 많습니다.

학습 뷰(`Ctrl+3`)의 실행 기록 표에도 행마다 **`ONNX`** 버튼이 있습니다. 이쪽은 **모델 가중치를 바꾸지
않고** 그 실행의 체크포인트로 바로 파일을 씁니다. 예전 실행을 꺼내 비교할 때 씁니다.
...
변환 12종: `크기 변경`, `흑백`, `잘라내기`, `정규화`, `범위 변환`, `원핫`, `argmax`, `softmax`, `임계값`,
`라벨 이름`, `JSON 포인터`, `문자 토큰화`.
...
학습률이 실제로 변할 때(스케줄이나 워밍업) 손실 곡선 아래에 **학습률 곡선**이 따로 그려집니다.
내내 같은 값이면 곡선이 아무것도 말해 주지 않으므로 그리지 않습니다. 손실과 자릿수가 달라(1e-3 대 1) 같은
...
아직 저장하지 않았으면 `학습 기록을 남길 폴더가 필요합니다 — 프로젝트를 먼저 저장하세요` 를 띄우고
**저장 대화상자를 바로 열어 줍니다.** 거기서 취소하면 학습도 시작하지 않습니다.
형상 오류가 있으면 시작하지 않고 문제 탭을 열어 줍니다.

진행 줄에 `에포크 50/50`, `학습 손실 0.1261`, `검증 손실 0.1098`, `정확도 0.9850` 이 나오고 아래 플롯에
...
**`가중치 적용`** 버튼이 그 실행의 가중치를 모델에 붙입니다. 정상적으로 끝난 학습은 자동으로
적용되므로, 이 버튼은 중간 실행으로 되돌릴 때 씁니다.

검증 집합이 있으면(검증 비율 > 0) 검증 손실이 낮아질 때마다 그 시점의 가중치가 `best.safetensors` 로
따로 남습니다. **`가중치 적용` 은 그것이 있으면 마지막이 아니라 그것을 씁니다** — 마지막 에포크가 이미
과적합으로 넘어간 뒤일 수 있기 때문입니다.

조기 종료로 끝난 실행은 결과 체크포인트 자체가 최적 가중치라서 체크포인트 열에 `(최고)` 가 붙습니다.
끝까지 돈 실행은 결과가 마지막 가중치라 표시가 없지만, `가중치 적용` 은 여전히 `best` 를 씁니다.

체크포인트는 safetensors 로 저장됩니다. 빌더는 프로젝트 **파일 이름**을 따 `<파일이름>.runs/<실행id>/`
아래에 두고(`demo.nlproj` → `demo.runs/`), 그 안에 `final.safetensors` 와 (주기를 정했으면)
`epoch-0005.safetensors` 가 생깁니다. 검증 집합이 있으면 `best.safetensors` 도 함께 남습니다.
명령줄 `nl train` 은 같은 폴더 대신 `runs/<실행id>/` 를 쓰고 `--run-dir` 로 바꿀 수 있습니다.
...
### 모델 노드 — 우리 모델과 가져온 ONNX

`모델` 서브메뉴에는 프로젝트의 모델 목록과, 그 아래 **`ONNX 파일에서…`** 가 있습니다.

| | 우리 모델 | 가져온 ONNX |
| --- | --- | --- |
| 어디서 오나 | 캔버스에서 만들고 학습시킨 것 | 바깥에서 받은 `.onnx` 파일 |
| 학습 | 됩니다 | **안 됩니다** — 추론 전용입니다 |
| 캔버스 | 레이어 그래프로 보입니다 | 보이지 않습니다. 파이프라인의 노드 하나로만 있습니다 |
| 고르는 법 | 목록에서 이름으로 | 인스펙터에 **파일 경로**를 적습니다 |
| 페이로드 | 없으면 모델에 적힌 것을 씁니다 | 없으면 텐서를 그대로 주고받습니다 |

경로는 **프로젝트 파일이 있는 폴더 기준 상대 경로**입니다. `..` 나 절대 경로는 거부합니다 —
남에게 받은 프로젝트가 내 컴퓨터의 아무 파일이나 읽지 못하게 하는 규칙이고, 모델 가중치에
적용되는 것과 같습니다.

빌드하면 `.onnx` 가 배포판 안(`assets/`)으로 들어가므로, 받는 사람은 파일을 따로 챙길 필요가
없습니다.

> **ONNX 를 켠 빌드가 필요합니다.** 실행은 `tract` 가 하는데 바이너리가 크게 늘어서 기본으로는
> 꺼져 있습니다. 꺼진 빌드에서도 **프로젝트는 그대로 열리고 저장됩니다** — 그 노드만
> "이 빌드는 ONNX 가져오기가 꺼져 있다" 로 실패하고 나머지 파이프라인은 돕니다.
> 켜서 빌드하려면 `cargo run --release -p nl-app --features onnx-import`.

입력·출력 순서는 **ONNX 파일에 적힌 순서**입니다. 형상이 맞지 않으면 실행할 때 알려 줍니다.
동적 배치(`dim_param`)로 내보낸 모델은 그대로 쓸 수 있습니다.
...
```sh
curl -H 'X-NL-Token: 토큰' -d '[0.8,-0.8]' http://127.0.0.1:8787/infer
curl -H 'Authorization: Bearer 토큰' -d '[0.8,-0.8]' http://127.0.0.1:8787/infer
```
...
![HTTPS 설정](img/11-tls.png)

노드 인스펙터의 **HTTPS** 절을 펼치고 `TLS 로 열기` 를 켭니다. 인증서와 개인키 PEM 경로 칸이 나오고
기본값은 `certs/cert.pem` 과 `certs/key.pem` 입니다.

| 칸·버튼 | 하는 일 |
| --- | --- |
| 인증서 PEM | 서버 인증서 사슬. 서버 것을 맨 앞에 두고 중간 인증서를 뒤에 |
| 개인키 PEM | 그 인증서의 키 |
| `고르기…` | 파일 선택. **프로젝트 폴더 밖 파일은 받지 않습니다** |

경로는 프로젝트 폴더 기준 **상대 경로**로 저장합니다. 절대 경로를 넣으면 붉게 막습니다. 다른 컴퓨터에서
그 프로젝트를 열면 없는 경로가 되고, 개인키를 홈 어딘가에서 끌어다 쓰는 습관이 그대로 배포 사고가 되기
때문입니다. 파일이 그 자리에 없으면 그것도 알려 줍니다.

자체 서명 인증서가 필요하면 빌더가 알려 주는 `nl tls-cert <프로젝트 폴더>` 명령을 복사해 쓰세요.
빌더가 키를 대신 만들지 않는 이유는, 키를 어디에 두고 누가 읽을 수 있게 할지가 여러분의 판단이어야
하기 때문입니다.

**인증서와 키는 배포물에 들어가지 않습니다.** 배포한 앱이 어디서 찾는지는
["인증서를 어디에 두는가"](#인증서를-어디에-두는가)를 보세요.

TLS 를 켜면 `바깥에서 불러 보기` 의 예시도 `https://` 로 바뀌고, 자체 서명을 받아들이는 `-k` 가 붙습니다.
제대로 된 인증서라면 `-k` 를 빼고, 사설 CA 라면 `--cacert ca.pem` 을 쓰세요.
...
모델에 `Output` 이 여럿이고 페이로드에도 출력 필드가 그만큼 있으면, 응답은 **필드 이름을 키로 하는
JSON 객체**가 됩니다. 출력이 하나면 예전처럼 값 하나가 그대로 나옵니다 — 기존 파이프라인의 응답은
한 글자도 바뀌지 않습니다.

```sh
# 출력 필드가 "class" 와 "score" 두 개인 모델
curl -d '[0.8,-0.8]' http://127.0.0.1:8787/infer
{"class":[0.12,0.88],"score":[0.91]}
```

**순서가 계약입니다.** 모델의 `Output` 노드와 페이로드의 출력 필드를 이름으로 맞추지 않고 순서로
맞춥니다. 그 순서는 `Output` 노드의 **이름순**입니다. 그래서 출력이 여럿이면 노드마다 이름을 주세요.
이름을 비워 두면 순서가 정해지지 않아 필드가 뒤바뀔 수 있습니다. 개수가 다르면 노드 오류입니다.

입력도 대칭입니다. 입력 필드가 여럿이면 같은 모양의 객체로 보냅니다.

```sh
curl -d '{"left":[1,2],"right":[3,4]}' http://127.0.0.1:8787/infer
```

배열로 보내면 **입력 필드 순서**대로 채웁니다(`[[1,2],[3,4]]`). 개수가 맞지 않으면 기대하는 필드
이름을 알려 주며 거절합니다. 입력 필드가 하나뿐이면 배열은 그 필드의 값 자체로 봅니다.

`Log`·`GUI 위젯`·`WebSocket 보내기` 싱크도 같은 객체를 그대로 JSON 으로 내보냅니다.
...
```sh
curl --cacert certs/server.crt -H 'X-NL-Token: 토큰' -d '[0.8,-0.8]' https://127.0.0.1:8787/infer
```
...
| 모델 출력 | 그 모델의 **마지막 추론 값** → 위젯. 싱크 노드를 따로 두지 않아도 됩니다 |
| 내장 동작 | `파이프라인 시작` / `파이프라인 정지` / `앱 종료` |

`모델 출력` 은 노드가 아니라 **모델**을 가리킵니다. 그 모델을 돌리는 노드가 값을 낼 때가 곧 "마지막
추론 값" 입니다. 페이로드에 출력 필드가 여럿이면 어느 필드를 보여 줄지도 고릅니다.
...
`▶ 미리보기` 를 켜면 런타임과 **같은 렌더러**로 바뀌고, 아직 도는 것이 없으면 선택한 파이프라인이
실제로 돕니다. 켜져 있는 동안 편집은 꺼집니다. 배포 전에 여기서 확인하세요.

옆에 **어느 파이프라인이 도는지** 적힙니다. 값 위젯이 `—` 인 채로 있다면 대개 지금 도는 파이프라인이
그 위젯을 먹이지 않는 것입니다 — 위젯을 먹이는 쪽(`GUI 위젯` 싱크나 `모델 출력` 바인딩)이 그 파이프라인
안에 있어야 합니다. 아무것도 안 돌고 있으면 그 자리에 그렇게 적힙니다.
...
./내앱 --log-json             # 로그 한 줄이 이벤트 하나인 JSON (journald·Loki)
./내앱 --work-dir ~/앱데이터  # 작업 폴더 고정 (서비스로 돌릴 때)
./내앱 --env-file app.env     # 토큰 같은 환경 변수를 파일에서
./내앱 다른앱.nlapp           # 다른 번들 실행
./내앱 --version
```

뒤의 셋은 서버로 운영할 때 쓰는 것이라 [서버로 운영하기](#서버로-운영하기)에서 자세히 다룹니다.
...
```
POST /infer  [0.8,-0.8] → 200  [-4.331, 5.576]
POST /infer  [1,1] → 200  [4.833, -3.780]
```

본문은 **필드 값 그대로**입니다. 입력 필드가 `x`(길이 2 벡터) 하나면 `[0.8,-0.8]` 을 보냅니다 —
`{"x":[0.8,-0.8]}` 처럼 감싸면 그 객체 전체를 `x` 의 값으로 읽어 인코딩에 실패합니다.
...
nl infer p.nlproj --model XOR --input '[0.8,-0.8]'      # JSON 값으로 추론
...
curl --cacert certs/server.crt -d '[0.8,-0.8]' https://127.0.0.1:8799/infer
```
...
| `[nl-app] runner: started` | 파이프라인 시험 실행이 시작됐다 |
| `[nl-app] runner: http <주소>` | 인바운드 HTTP 서버가 열렸다 — **요청은 이 뒤에 보낸다** |
| `[nl-app] runner: error <노드> <메시지>` | 실행기가 오류를 냈다 |
| `[nl-app] runner: stopped` | 실행기가 멈췄다 |

`runner:` 줄에 값 이벤트는 넣지 않았습니다. 같은 오류가 틱마다 다시 와도 **바로 앞과 같은 줄이면
건너뜁니다**. 로그가 부풀면 기다리는 쪽이 느려지기 때문입니다.
