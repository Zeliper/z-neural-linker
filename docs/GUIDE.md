...
| HTTP 서버 | **`127.0.0.1:8787`**, 경로 **`/infer`** |
...
>
> **킬 스위치는 빌더에만 있습니다.** 배포판(`nl-runtime`)은 사용자가 만든 앱이므로 마우스/키보드 싱크가
> **무장된 채로 시작**하고 킬 스위치가 없습니다. 무장 싱크가 든 앱을 배포할 때는 GUI 에 정지 버튼을
> 꼭 넣으세요.

### HTTP 서버 노드

`HTTP 서버` 소스와 `HTTP 응답` 싱크를 짝지으면 파이프라인이 **추론 API** 가 됩니다.

인스펙터에서 주소를 확인하세요. `127.0.0.1` 이면 `✔ 루프백 — 이 컴퓨터에서만 닿습니다`,
바깥에 열린 주소면 `⚠ 바깥에서 닿을 수 있는 주소입니다 — 방화벽을 확인하세요` 가 뜹니다.

**바깥에서 불러 보기** 칸에 그대로 쓸 수 있는 명령이 있습니다.

```sh
curl -X POST http://127.0.0.1:8787/infer -d '[0,1]'
```

- 요청 본문이 값이 됩니다. 본문이 비어 있으면 쿼리스트링이 JSON 객체가 됩니다 (`?a=1&b=hi` → `{"a":"1","b":"hi"}`).
- 응답은 같은 파이프라인의 `HTTP 응답` 싱크가 돌려줍니다. 짝이 없으면 요청이 제한 시간까지 기다립니다.
- 받는 메서드는 `GET`·`POST`·`PUT`·`PATCH`, 본문 상한은 8 MiB, 응답 제한 시간은 10초(초과 시 504)입니다.
- **요청은 한 번에 하나씩** 처리합니다. 동시에 여러 개가 와도 도착 순서대로 한 틱에 하나씩 돌므로,
  `틱` 속도가 곧 초당 처리량의 상한입니다.
...
`배포 앱 자동 업데이트` 를 채우면 만든 앱이 스스로 새 버전을 확인합니다. 매니페스트 주소를 비우면
자동 업데이트가 꺼집니다.
...
같이 만들어지는 `latest.json` 이 자동 업데이트 매니페스트입니다. 자산 기본 주소를 비워 두면 URL 이 파일
이름뿐이니 올릴 때 앞에 주소를 붙이세요.
...
`NL_UPDATE_URL` 환경 변수가 번들에 적힌 매니페스트 주소보다 우선합니다.
